...
#paivitaPelaajanTiedot, haeLentokentanTiedot, haeMaanTiedot, haePelaajanTiedot, haeMaanLentokentat, haeLentokentat
...
async def lenna(maaranpaa, sijainti, paivitaPelaaja, pelaajanID):
    #haetaan sijainnin ja määränpään koordinaatit
    maaranpaaKoordinaatit = await tietokanta.haeLentokentanTiedot("latitude_deg, longitude_deg", "ident", maaranpaa)
    sijaintiKoordinaatit = await tietokanta.haeLentokentanTiedot("latitude_deg, longitude_deg", "ident", sijainti)
    ...
    if paivitaPelaaja == 1:
        ...
        await tietokanta.paivitaPelaajanTiedot(pelaajanID, "location", maaranpaa)
        await tietokanta.paivitaPelaajanTiedot(pelaajanID, "co2_consumed", (co2Lennolta + pelaajanTiedot["co2_consumed"]))
...
async def laskeLennonPituus(lahtosijainti, maanosa):
    ...
    lahtosijainti = await tietokanta.haeLentokentanTiedot("latitude_deg, longitude_deg", "ident", lahtosijainti)
    #haetaan kaikki maanosan maat
    lentokentat = await tietokanta.haeLentokentat()
    tulos = []
    start = time.process_time()
    for lentokentta in lentokentat:
        #lasketaan matkan pituus ja co2 päästöt
        matkanpituus = distance.distance(lahtosijainti, await tietokanta.haeLentokentanTiedot("latitude_deg, longitude_deg", "ident", lentokentta[0])).km
        co2Lennolta = co2 * matkanpituus
